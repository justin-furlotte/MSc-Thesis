...
Thus $\Delta Q = -\sigma_H \Delta \phi$. We now argue that the Hall conductivity must be an integer multiple of $q^2/h$. Suppose that we now unroll the cylinder to work in Cartesian coordinates, but still associating $x=0$ and $x=L$ with periodic boundary conditions. We choose the \emph{Landau gauge} for the magnetic field,
...
The fact that the Hamiltonian commutes with the $y$-momentum is evident, and allows us to replace $p_y$ with its eigenvalue, $\hbar k$. The wavenumber is quantized by the periodic boundary condition

\[e^{i0k} = e^{i 2\pi R k},\]

which implies $p_y = \hbar k = \hbar n/R$, giving

\[\begin{aligned}
H &= \frac{1}{2m}\left(p_x^2 + \left( \frac{\hbar n}{R} - q\left( Bx + \frac{\phi}{2\pi R}\right)\right)^2\right)\\
&= \frac{1}{2m}\left(p_x^2 + q^2B^2 \left(x -  \frac{1}{2\pi RB}\left(\frac{h}{q}n - \phi \right)\right)^2\right).
\end{aligned}\]

Define the frequency $\omega = \frac{qB}{m}$, and the shift factor

\[s_n := \frac{1}{2\pi RB}\left(\frac{h}{q}n - \phi \right).\]

In this notation, the Hamiltonian takes the form

\[H = \frac{1}{2m}p_x^2 + \frac{1}{2}m\omega^2(x-s_n)^2.\]

This is a shifted quantum harmonic oscillator in $x$, with frequency $\omega$. The eigenstates of $H$ are called \emph{Landau levels}, and they are exponentially localized at $x-s_n$, with their square modulus taking the form

\[\norm{\psi_n(x,y)}^2 = c_n H_n(x-s_n)e^{-\frac{(x-s_n)^2}{2\ell_B^2}}\]

Where $\ell_B^2 = \frac{\hbar}{qB}$, $c_n$ are normalization constants, and $H_n$ are the Hermite polynomials. The energies are given by (need to add)

The crucial argument now comes from inspecting the shift $s_n$. Suppose we adiabatically increase $\phi$ by one \emph{flux quantum}, $\phi_0 \mapsto \phi_0+\frac{h}{q}$, as time evolves from $t=0$ to $t=T$. The adiabatic principle tells us that the eigenstates of the Hamiltonian at $t=0$ must also be an eigenstate of the Hamiltonian at $t=T$. But we can find exactly what the new eigenstate is; the only thing that changes is the shift, which by a simple calculation becomes

\[s_n \mapsto s_{n-1}.\]

The (exponentially localized) wavefunctions are each transported to the left in increments of $s_1=\frac{h}{2\pi qRB}$. Heuristically, the diagram in figure (need to add) explains why charge transport occurs. Inspect one filled Landau level. On the right, the single empty state just above the Fermi energy is brought below the Fermi energy. On the left, the single filled state just below the Fermi energy is brought above the Fermi energy. Everywhere in between, states are merely transported to the left, each one filling the other's place. Thus, exactly one charge $q$ is transported from the right edge to left edge. This occurs once per Landau level, and thus the total charge transported is 

\[\Delta Q = -q N_{L}\]

where $N_{L}$ is the (integer) number of filled Landau levels. Our result $\Delta Q = -\sigma_H \Delta \phi$ now gives us exactly the formula which agrees with experiment,

\[\sigma_H = \frac{q^2}{h}N_{L} \quad\quad\quad\quad N_{L}\in\N.\]

\subsubsection{Halperin's Argument}

Need to add.

\subsubsection{Bulk-Edge Correspondence - Laughlin and Halperin Pictures}
