...
Indeed, taking the derivative of the right hand side gives $i[\Delta H_B(t), \rho_I(t)]$, and the constant $P_\mu$ is added so that the initial condition $\norm{\rho_I(t) - P_\mu} \to 0$ is also satisfied. This also shows that

\[\norm{\rho_I(t)-P_\mu} \leq 2\int_{-\infty}^t \norm{V_0\Lambda_2 e^{\eps s} \rho_I(t)}ds = e^{\eps t}\mathcal{O}(V_0),\]

which implies $[\Delta H_B(s), \rho_I(t)] = [\Delta H_B(s), P_\mu] + e^{2\eps t}\mathcal{O}(V_0^2)$. Therefore

\[\rho_I(t) = i\int_{-\infty}^t [\Delta H_B(s), P_\mu]ds + P_\mu + e^{2\eps t}\mathcal{O}(V_0^2).\]
...
where we used the fact that the error $e^{2\eps t}\mathcal{O}(V_0^2)$ introduced by replacing $\rho_I$ with $P_\mu$ vanishes in the limit $V_0\to 0$ at $t=0$. Some simplifications can be made,
...
The norm of the resolvent is bounded by $\norm{R_a(z)} \leq C|\text{Im}(z)|^{-1}$. The middle operator is bounded by Lemma \ref{lemma:keybound}. For the operator on the right, we apply the Combes-Thomas bound \ref{thm:combesthomasresolvent}. Altogether, the bound of the first term of the integrand takes the form 
...
We may therefore replace $\rho(H_B)$ with $P_\lambda$, by which we obtain $\Tr(\Sigma_B) = i\Tr(P_+[P_\lambda,\Lambda_1]\Lambda_2P_+) + i\Tr(P_-[P_\lambda,\Lambda_1]\Lambda_2P_-)$. We must relate this expression to the bulk conductivity. An algebraic calculation shows that that 
...
We denote $N = \sup_L{\mathcal{H}_L}$. The $C^*$-algebra $\U_X$ of observables on $\mathcal{H}_X$ consists of bounded self-adjoint operators supported in $X$. For an operator $A_X \in \U_X$, we identify its extension to an operator on $\mathcal{H}_L$ by taking its tensor product with copies of the identity, $(\otimes_{x \notin X} \mathbb{I}_x) \otimes A_X$. Conversely, we say that an operator $A \in \mathcal{U}_L$ has support $X$ if $A_X := (\otimes_{x \notin X} \mathbb{I}_x)\otimes (A|_{X})$ is equal to $A$, and write $A_X \in \U_X$. For ease of notation, we omit the subscript $L$ wherever there is no risk of confusion.
...
We consider a region as depicted in Figure~\ref{fig:setup}, with the left and right edges joined together to form a cylinder. 

\subsection{The Edge System}
In Figure \ref{fig:setup}, $H_0=0$ is a trivial Hamiltonian which we take to be empty space, supported in the left white region $[0,L/2] \times [0,L]$, while in the right blue region $[L/2,L]\times [0,L]$, $H_1$ is a so-called \textit{local} or \textit{finite-range Hamiltonian}, in the sense that $H_1 = \sum_{X \subseteq \Gamma_L} \Phi(X)$, is a sum of local interactions. This replaces assumption \ref{ass:shortrange}. We define the Hamiltonian of the full edge system to be 

\[H_E(\mu) = H_1 + \mu Q_h,\]

where $Q_h = \sum_{x \in \Gamma_h} a_x^*a_x$ is the \emph{number} or \emph{charge} operator for the region $\Gamma_h = [L/4,3L/4] \times [0,L]$ shown in red. This introduces a driving strength; the $\mu Q_h$ term can be viewed as a potential difference $V(x)$. We emphasize that $H_1$ is supported on $[L/2,L]\times[0,L]$.
...
The bulk system is again modelled by the cyclinder $\Gamma_L$, except this time, the Hamiltonian $H_B$ is nonzero on the entire cylinder (as opposed to just on $[L/2,L]\times [0,L]$, as was the case with the edge Hamiltonian). The associated ``bulk" Hamiltonian is also assumed to be local, $H_B(\mu) = \sum_{X\subseteq \Gamma_L}\Phi(X) + \mu Q_h$, where this time $Q_h$ is the number operator on $(-\infty,L/2)\times (-\infty,\infty)$. The exact same finite box-shaped regions $\Gamma_h$, $\Gamma_u$, $\Gamma_E$, $\Gamma_B$, and $\Gamma_m$ in Figures \ref{fig:setup} and \ref{fig:regions} are also present in the bulk setting on $\Z^2$; in particular, they are not extended to infinity in any direction. 
...
$H_E$ is assumed to have a bounded spectrum, while $H_B$ has a spectrum which is both bounded and gapped, as in assumption \ref{ass:gap}. We also assume that the bulk and edge Hamiltonians are both \emph{locally charge-conserving}.
...
\fill[pattern=north west lines, pattern color=white!80] (2.25,0) rectangle (3.75,12);
\node[color=white] at (3,1) {$\Gamma_B$};
...
To summarize, the bulk and edge Hamiltonians are both finite-range, and both follow assumptions \ref{ass:charge} and \ref{ass:bulk}, rather than \ref{ass:shortrange} and \ref{ass:edge}. Additionally, $H_B$ satisfies \ref{ass:gap}.
...
Let $Q_u = \sum_{x \in \Gamma_u} a_x^* a_x$ be the charge in the upper half of the cylinder $\Gamma_u = [0,L] \times [L/2,L]$ (the yellow region in Figure \ref{fig:setup}), and define current operator of the edge system,
...
\begin{lemma}
The ground state expectation of the both the bulk and edge system current $J^B$ and $J^E$ is zero.
\label{lemma:J=0}
\end{lemma}
...
We return to the analogy of $\mu Q_h$ as an electric potential. Let $P = P_E,P_B$ and $J=J_E,J_B$ denote either the edge or bulk system projection and current. Increasing the potential by a small amount $d\mu Q_h$ and expanding to linear order, the change in ground state current is given by
...
Now, recalling the region $\Gamma_h$ in Figure \ref{fig:setup}, $[H_B, Q_h]$ is supported on a strips of width $2R$ centered at $x=L/4$ and $x=3L/4$, and thus for large enough $L$, it is supported on $\Gamma_B$. On the other hand, $J_V^B$ is supported on $V$. By Lemma~\ref{lemma:local}, the commutator $[\mathcal{I}([H_B,Q_h]), \mathcal{I}(J_V^B)]$ can be written
...
The intuitive picture from the previous result is that the Hall conductivity of the edge system is essentially only nonzero along $\Gamma_E$ and $\Gamma_B$. The ground state expectation of the edge current $\Tr(PJ)$ is zero by Lemma~\ref{lemma:J=0}. If there is a Hall current flowing along the strip $\Gamma_B$ where the driving $\mu Q_h$ occurs, then there must therefore be an equal current flowing along the edge strip $\Gamma_E$, but in the opposite direction; $\Tr(PJ_{\Gamma_E}) = -\Tr(PJ_{\Gamma_B}) + \mathcal{O}(L^{-\infty})$.
...
We conclude using assumption \ref{ass:bulk} that the Hall conductivity $-\Tr(\dot{P} J_{\Gamma_B})$ in the region $\Gamma_B$ is equal (up to $\mathcal{O}(L^{-\infty})$) to that of the actual bulk system, $\sigma^B_{\Gamma_B} = -\Tr(\dot{P}_BJ^B_{\Gamma_B})$. We therefore have bulk-edge correspondence, in the sense that 
...
Of course, the ultimate goal would be to show that the ground state current is zero in the system with an edge, which is what allowed us to conclude bulk-edge correspondence in the cylinder case. However, using our previous results, we can at least show a simple proof of this in the non-interacting case for the edge system.
...
\section{Projection-Values Measures}\label{sec:pvm}
...
where $f:\Z^2\to\R$ is any Lipschitz function whose Lipschitz constant is not grater than 1, $f \in \text{Lip}^1$, and $\eps$ can be chosen as $\eps = \frac{1}{C(1+|\text{Im}(z)|^{-1})}$. 
