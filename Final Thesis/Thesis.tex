...
One of the first explanations employing quantum mechanics is due to Laughlin in his short but seminal 1981 paper~\cite{Laughlin}, which explained the plateaux using a charge transport argument. 
...
The reasoning above only guarantees the correct value of $\sigma_H$ if the Landau levels are exactly filled. Disorder, i.e. the presence of impurities in the material, is required to explain why plateaux occur. We model disorder using a random potential. 

Under the addition of a suitable random potential to the Hamiltonian, the degeneracy of the Landau levels is lifted; they broaden into bands, but do not overlap. Furthermore, while the center of the bands may be a continuous spectrum, the edges of the bands are pure-point (need to add reference to Combes Hislop); this phenomenon is called Anderson localization. These pure-point states, called \emph{localized states}, contribute nothing to the current, as they have compact support which does not wrap around the cylinder. The effect of $\phi$ on these states can therefore be gauge transformed away, since the domain of their support is simply connected, ensuring that $\vec{A}_\phi$ is a (locally) conservative vector field. This is not the case on the cylinder as a whole, and in particular it is not the case for \emph{extended states}, whose domain is not simply connected.

As the magnetic field strength $B$ decreases, the degeneracy of each Landau level, $N_s = \frac{q}{h}BA$, also decreases. These extra states begin to populate localized states, which contribute nothing to the current.

Indeed, as $B$ decreases, so too does the Fermi energy $\mu$ (need to add). No states are added to the system so long as the Fermi energy remains in a gap, but once $\mu$ reaches the edge of a conduction band, state are added to the system, but by Anderson localization they have pure point spectrum and leave the current unchanged; this is what gives rise to the plateaux. Finally, once the Fermi energy reaches the continuous part of the spectrum, the current changes, and the Hall conductivity jumps to another plateau.
