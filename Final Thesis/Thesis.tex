...
\begin{assumption}
The Hamiltonians $H_E$ and $H_B$ are both sums of local interactions $\sum_X\Phi(X)+\mu Q_h$, and the each interaction $\Phi(X)$ conserves the total charge, $[\Phi(X),Q] = 0$, where $Q=\sum_{x\in\Gamma_L}a^*_xa_x$ is the total charge in $\Gamma_L$.
\label{ass:charge}
\end{assumption}
