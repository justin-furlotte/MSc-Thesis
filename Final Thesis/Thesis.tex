...
\end{proposition}
\begin{proof}
We begin with the Heisenberg equation of motion for the density matrix, $\dot{\rho}(t) = - i[\widetilde{H}_B(t),\rho(t)]$, with initial condition $\lim_{t\to-\infty} \|\rho(t) - e^{itH_B}P_\lambda e^{-itH_B}\| = 0$, which is equivalent to $\lim_{t\to-\infty} \|e^{-itH_B}\rho(t)e^{itH_B} - P_\lambda\| = 0$.
...
which implies $[\Delta H_B(s), \rho_I(t)] = [\Delta H_B(s), P_\lambda] + e^{2\eps t}\mathcal{O}(V_0^2)$. Therefore
...
We return to the analogy of $\mu Q_h$ as an electric potential. Let $P = P_E,P_B$ and $J=J^E,J^B$ denote either the edge or bulk system projection and current. Increasing the potential by a small amount $d\mu Q_h$ and expanding to linear order, the change in ground state current is given by
...
Taking a derivative, and recalling that $J^E_{\Gamma_B} = J^B_{\Gamma_B}$, we see the edge Hall conductivity $\Tr(\dot{P}_EJ^E_{\Gamma_E})$ is equal to

\[\sigma^E_{\Gamma_E} =-\Tr(\dot{P}_E J^B_{\Gamma_B}) + \mathcal{O}(L^{-\infty}).\]

We conclude bulk-edge correspondence using assumption \ref{ass:bulk}, in the sense that the edge Hall conductivity is equal (up to tails) to the bulk conductivity,
