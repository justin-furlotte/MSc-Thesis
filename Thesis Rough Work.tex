...
Before giving the proof in its entirety, we outline the basic steps. The key ingredient is the use of the funcional calculus given by the Helffer-Sjostrand representation of self-adjoint operators on a Hilbert space (need to add reference to appendix here).
...
This operator has zero trace. Indeed, the first term has vanishing trace in the position basis, while the second term's integrand involves the trace of $[R,R]=0$. So $\sigma_E(a) = \Tr(\Sigma(a))$, where
...
Using the Hellfer-Sjostrand representations for the first two terms on the right hand side, we obtain
...
since $H_a$ is self-adjoint. The norm of the second operator can be bounded by inspecting its matrix elements.
...
For any $n\in \N$, the quasi-analytic extension $\tilde{\rho}$ of $\rho$ in the Helffer-Sjostrand representation can be chosen so that 
...
We appeal to the general fact of functional analysis that strong convergence of the resolvent implies that $J_a f(H_a) J_a^* \xrightarrow{\enskip s \enskip} f(H_B)$ for any bounded and continuous function $f$. In particular, the functions $[(\cdot - z)^{-1}, \Lambda_1]$ and $[\cdot, \Lambda_2]$ above are bounded and continuous, so we will have proven the desired limits if we can prove convergence of the resolvent, $J_aR_a(z)J_a^* \xrightarrow{\enskip s \enskip} R_B(z)$.
...
\[J_aR_a(z) - R_B(z)J_a = -R_B(z)E_aR_a(z).\]

Taking the adjoint, and then multiplying from the left by $J_a$, we see that 

\[\begin{aligned}
J_aR_a(z)J_a^* - R_B(z) &= -J_aR_a(z)E_a^*R_B(z)\\
&= -(J_aR_a(z)E_a^* + 1 - J_aJ_a^*)R_B(z)\\
& \xrightarrow{\enskip s \enskip} 0,
\end{aligned}\]

since $E^*_a \xrightarrow{\enskip s \enskip} 0$ by the edge assumption, and $1 - J_aJ_a^* \xrightarrow{\enskip s \enskip}  0$. This proves that the limits above converge to the desired associated bulk operators, and thus $\Sigma(a) \xrightarrow{\enskip s \enskip}  \Sigma_B$.
...
Since $P_\pm R_B(z)$ and $R_B(z) P_\pm$ are analytic on $\text{supp}(\rho(z))$ and $\text{supp}(1-\rho(z))$, respectively, the integral in $P_\pm \Sigma_B P_\pm$ vanishes by integration by parts. Thus
...
\begin{assumption}
Define the bulk region $\Gamma_B = [3L/4-k, 3L/4+k] \times [0,L]$ for some $k>0$. For any operator $A \in \U_{\Gamma_B}$,
\[\emph{Tr}(PA) = \emph{Tr}(P_BA) + \mathcal{O}(L^{-\infty}).\]
\label{ass:bulk}
\end{assumption}

The idea is that observables localized far away from the edge are not affected by the edge of the system.
...
We also recall a definition from the non-interacting setting: an \emph{off-diagonal} operator is an operator $A$ such that $A = \overline{A} := P_\mu AP_\mu^\perp + P_\mu^\perp AP_\mu$, where $P_\mu^\perp = \mathbb{I} - P_\mu$ is the projection onto the excited states above the gap.

\begin{lemma}
For any off-diagonal operator $A=\overline{A}$, $\mathcal{I}_\mu(\cdot)$ and $[H_\mu, \cdot]$ act as inverses of each other, up to a factor of $i$:

\[\mathcal{I}_\mu\left([H_\mu, A]\right) = [H_\mu, \mathcal{I}_\mu(A)] = iA.\]

Furthermore, for any (not necessarily off-diagonal) operator $A$, 
...
where $X^r$ is the $r$-fattening of $X$.
\label{lemma:local}
\end{lemma}
...
\end{proposition}
\begin{proof}
Since $\dot{H}_\mu=Q_h$, we see that for any $\mu_1$ and $\mu_2$
...
since $Q_h$ and $Q_u$ commute. The proof of Lemma \ref{lemma:J=0} provides the necessary bounds to invoke linearity and cyclicity of the trace to shift the commutator in the second line and the second-last line. (need to add a bound for $\norm{\dot{P}}_1$).
\end{proof}
